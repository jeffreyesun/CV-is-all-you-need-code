...
"Transpose two elements of a vector"
function transposeelements(v::AbstractVector, i, j)
    v[i], v[j] = v[j], v[i]
    return v
end
transposeelements(t::Union{Tuple,UnitRange}, i, j) = Tuple(transposeelements(collect(t), i, j))

function get_perm(n, transpositions)
    perm = collect(1:n)
    for t in transpositions
        transposeelements(perm, t[1], t[2])
    end
    return perm
end
...
function transposedims!(A, B, i, j=1)
    ndims_A = ndims(A)
    @assert ndims_A == ndims(B)
    return permutedims!(A, B, transposeelements(1:ndims_A, i, j))
end

get_mat_dims(t, rowdim=1) = (nrows = t[rowdim]; (nrows, prod(t)÷nrows))
get_mat_dims(A::AbstractArray, rowdim) = get_mat_dims(size(A), rowdim)
...
#TODO Support different sized input and output
"Left multiply matrix `right` by matrix `left_mat` along dimension `dim` and store in `output`. Requires a buffer for temporary storage."
function matmul_slices!(output, left_mat, right, dim, left_buffer, right_buffer=output)
    right_mat = reshape(right_buffer, get_mat_dims(right, dim))
    output_mat = reshape(left_buffer, get_mat_dims(output, dim))
    right_perm = reshape(right_buffer, transposeelements(size(right), dim, 1))
    output_perm = reshape(left_buffer, transposeelements(size(output), dim, 1))

    transposedims!(right_perm, right, dim, 1)
    mul!(output_mat, left_mat, right_mat)
    transposedims!(output, output_perm, dim, 1)
    return output
end
